...
def get_random_nodes(graph, n):
    return np.random.choice(graph.nodes(), size=n,
                            replace=(len(graph.nodes()) < n))


def get_nodes_with_degree_percentile(graph, frm, to):
...
def get_nodes_with_lowest_degree(graph, n):
    nodes = sorted(graph.degree, key=lambda x: x[1])
...
def get_nodes_with_lowest_eigenvector_centrality(graph, n):
...
def get_nodes_with_lowest_betweenness_centrality(graph, n):
...
def get_nodes_with_lowest_closeness_centrality(graph, n):
...
def get_nodes_with_lowest_pagerank(graph, n):
